...
const supabaseUrl = 'https://vuppdvlpsbyzhcmalhoi.supabase.co'
const supabaseKey = 'sb_publishable_frIjzGAOaW77jegazxN-_Q_OUqN-hzO'
...
// 2. Функция ЗАГРУЗКИ отзывов из Supabase
async function loadReviews() {
  ...
  // Безопасное отображение отзывов через textContent (защита от XSS)
  reviews.forEach(item => {
    ...
    nameEl.textContent = item.name || 'Аноним';

    const starsEl = document.createElement('span');
    starsEl.className = 'stars';
    starsEl.textContent = '⭐'.repeat(item.rating || 5);
    ...
    textEl.textContent = item.review || '';
...
// 3. Функция ОТПРАВКИ отзыва в Supabase
if (form) {
  form.addEventListener('submit', async (event) => {
    ...
    // Отправляем запись в Supabase
    const { error } = await supabase
      .from('reviews')
      .insert([{ name: name, rating: rating, review: review }]);
...
// Загружаем отзывы при запуске страницы
loadReviews();
// ==========================================
