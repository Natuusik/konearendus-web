...
const SUPABASE_URL = 'https://vuppdvlpsbyzhcmalhoi.supabase.co';
const SUPABASE_KEY = 'sb_publishable_frIjzGAOaW77jegazxN-_Q_OUqN-hzO';
...
form.addEventListener('submit', async (event) => {
  ...
  const rating = Number(document.getElementById('user-rating').value);
  ...
  if (error) {
    ...
    // Очищаем форму
    form.reset();
    // Перезагружаем список отзывов, чтобы сразу увидеть новый!
    await loadReviews();
  }
